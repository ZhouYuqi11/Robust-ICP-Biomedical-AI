# -*- coding: utf-8 -*-
"""
Pneumonia diagnosis experiment for robust ICP under data scarcity.

Experimental settings:
- Dataset: released PneumoniaMNIST/BiomedCLIP feature partitions.
- Available training pool: Train + Validation partitions.
...
DATASET_NAME = "Pneumonia diagnosis"
...
def load_npy_dataset() -> Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
    """Load released train/validation/test feature partitions from the current folder."""
    base = script_dir()
    required = {
        "X_train": "Train_image_features.npy",
        "y_train": "Train_labels.npy",
        "X_val": "Validation_image_features.npy",
        "y_val": "Validation_labels.npy",
        "X_test": "Test_image_features.npy",
        "y_test": "Test_labels.npy",
    }
    paths = {k: os.path.join(base, v) for k, v in required.items()}
    missing = [p for p in paths.values() if not os.path.exists(p)]
    if missing:
        raise FileNotFoundError("Missing required pneumonia files:\n" + "\n".join(missing))
    X_train = np.load(paths["X_train"], allow_pickle=False)
    y_train = np.load(paths["y_train"], allow_pickle=False).reshape(-1).astype(int)
    X_val = np.load(paths["X_val"], allow_pickle=False)
    y_val = np.load(paths["y_val"], allow_pickle=False).reshape(-1).astype(int)
    X_test = np.load(paths["X_test"], allow_pickle=False)
    y_test = np.load(paths["y_test"], allow_pickle=False).reshape(-1).astype(int)
...
def main():
    ...
    # Keep the released test set fixed. Only Train + Validation are used as the available pool.
    X_train, y_train, X_val, y_val, X_test, y_test = load_npy_dataset()
    X_pool_all = np.vstack([X_train, X_val])
    y_pool_all = np.concatenate([y_train, y_val]).astype(int)

    # Fit preprocessing on the available training pool only; never fit on the fixed test set.
